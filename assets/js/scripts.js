...
(function ($) {
    ...
    function ssubmitForm() {
        ...
        var employer = $("#semployer").val();

        $.ajax({
            type: "POST",
            url: "scripts/sign-up.php",
            data: "email=" + email + "&name=" + name + "&password=" + password + "&phone=" + phone + "&terms=" + terms + "&employer=" + employer,
    ...
    function sformSuccess() {
        $("#signUpForm")[0].reset();
        ssubmitMSG(true, "注册完成！系统将在3s后跳转至登录界面...");
        $("input").removeClass('notEmpty'); // resets the field label after submission
        setTimeout(function () {
            window.location.href = "https://tool2-mml.sjtu.edu.cn/yxshen_tmp/webserver/login.html";
        }
            , 3000);
    }
    ...
    function lformSuccess() {
        $("#logInForm")[0].reset();
        lsubmitMSG(true, "登陆成功!系统将在3s后跳转至主页...");
        $("input").removeClass('notEmpty'); // resets the field label after submission
        setTimeout(function () {
            window.location.href = "https://tool2-mml.sjtu.edu.cn/yxshen_tmp/webserver/index.php";
        }
            , 3000);
    }
