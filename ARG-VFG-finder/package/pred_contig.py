...
def getfasta(runID, infile):
	...
	lendict = {}
	lenout = os.path.join(final_dir,runID+'_length.json')
	lenlist.sort(reverse=True)
	lendict['data'] = lenlist
	jslen = json.dumps([lendict])
	with open(lenout,'w') as outlen:
		outlen.write(jslen)			
#	return id_dict,dict(virald_r,**locdict),dict(virald_o,**locdict)
	return id_dict, dict(virald_o,**locdict)

def _contig(runID,infile):
	...
	tag_file = os.path.join(tag_dir, f"co_{runID}")
...
if __name__ == '__main__':
	infile = argv[1]
#	runID = randomID(10)
	runID = "Example_Contig"
	_contig(runID,infile)
